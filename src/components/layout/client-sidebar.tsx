        ...
        <nav className="client-nav">
          ...
          {[1, 2, 3, 4, 5, 6, 7].map((i) => (
            <div key={i} className="client-nav-item" style={{ opacity: 0.3, pointerEvents: 'none' }}>
              ...
              {!isCollapsed && <div style={{ width: 80, height: 14, background: 'rgba(255,255,255,0.2)', borderRadius: 4 }}></div>}
            </div>
          ))}
        </nav>
      </aside>
    ...
    <aside className={`client-sidebar ${isCollapsed ? 'collapsed' : ''}`}>
      ...
      <nav className="client-nav">
        <Link
          ...
          title={isCollapsed ? (isOnboarding ? 'Getting Started' : 'Welcome') : undefined}
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"></path>
            <path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"></path>
          </svg>
          {!isCollapsed && <span>{isOnboarding ? 'Getting Started' : 'Welcome'}</span>}
        </Link>
        <Link
        ...
        >
          ...
          {smartRecsCount > 0 && (
            <span className={`nav-badge count${pathname === '/recommendations' ? ' active' : ''}${isCollapsed ? ' collapsed' : ''}`}>{smartRecsCount}</span>
          )}
        </Link>
        <Link
        ...
        <Link href="/login" className="client-nav-item nav-logout" title={isCollapsed ? 'Logout' : undefined}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
