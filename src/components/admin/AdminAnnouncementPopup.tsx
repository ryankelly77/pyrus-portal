...
function getAdminPageFromPathname(pathname: string): string {
  // Map admin paths to page keys
  ...
  if (pathname.startsWith('/admin/clients/performance') || pathname.startsWith('/admin/performance')) return 'performance'
...
export function AdminAnnouncementPopup() {
  ...
  useEffect(() => {
    ...
    const logView = async () => {
      try {
        await fetch(`/api/admin/announcements/active/${announcement.id}/view`, { method: 'POST' })
      } catch (error) {
        console.error('Failed to log announcement view:', error)
      }
    }
    logView()
  }, [loading, announcements, currentIndex])

  function filterByFrequency(items: Announcement[]): Announcement[] {
    return items.filter((announcement) => {
      const sessionKey = `admin_announcement_shown_${announcement.id}`
      const dayKey = `admin_announcement_day_${announcement.id}`
  ...
  function markAsShownForSession(id: string, frequency: string) {
    ...
    if (frequency === 'once_per_session') {
      sessionStorage.setItem(`admin_announcement_shown_${id}`, 'true')
    }
    if (frequency === 'once_per_day') {
      localStorage.setItem(`admin_announcement_day_${id}`, new Date().toISOString())
    }
  }
  ...
  const dismissAnnouncement = async (permanent: boolean = false) => {
    ...
    try {
      const res = await fetch(`/api/admin/announcements/active/${announcement.id}/dismiss`, {
        method: 'POST',
          ...
          <div className="popup-actions">
            ...
            {announcement.cta_button_text && announcement.cta_button_url && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="popup-btn popup-btn-cta"
              >
                {announcement.cta_button_text}
              </a>
            )}
